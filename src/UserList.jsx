    ...
    if (vastaus === true) { 
      ...
      .then(u => {
        if (u.status === 200) {
          ...
          setTimeout(() => {setShowMessage(false)},5000) // Ilmoituksen piilotus
          reloadNow(!reload)
        }
      })
      .catch(error => {
               ...
               const lowerCaseName = u.lastname.toLowerCase()
                 if(valueaccesslevelId === "1"){
                   if (lowerCaseName.indexOf(search) > -1) {                     
                      return (
                        <div className='showAllUsers' key={u.userId}>
                          <h4 onClick={() => toggleUserDetails(u.userId)}>
                            {u.firstname} {u.lastname}
                          </h4>
                          {showDetails[u.userId] &&
